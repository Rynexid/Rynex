...
import { getApprovedReviews, type Review } from "@/server/actions/reviews";

const avatars = [
  "/website-assets/04-testimonial-avatars/avatar-1.svg",
  "/website-assets/04-testimonial-avatars/avatar-2.svg",
  "/website-assets/04-testimonial-avatars/avatar-3.svg",
  "/website-assets/04-testimonial-avatars/avatar-4.svg",
  "/website-assets/04-testimonial-avatars/avatar-5.svg",
  "/website-assets/04-testimonial-avatars/avatar-6.svg",
];
...
export function TestimonialsSection() {
  ...
  useEffect(() => {
    getApprovedReviews()
      .then((reviews: Review[]) => {
        if (reviews.length > 0) {
          const mapped = reviews.map((r, idx) => ({
            quote: r.content,
            author: r.authorName,
            role: r.authorRole ?? "Pelanggan Rynex",
            rating: r.rating,
            avatar: avatars[idx % avatars.length],
          }));
          setTestimonials([...mapped, ...fallbackTestimonials]);
        }
      })
      .catch(() => {});
  }, []);
          ...
          {[...testimonials, ...testimonials].map((item, i) => (
            ...
            >
              <div className="hover:border-primary/20 border-border bg-card hover:bg-card/80 h-full rounded-2xl border p-5 transition-[background-color,border-color] duration-300 md:p-6">
                ...
                <div className="border-border/50 flex items-center gap-3 border-t pt-3 md:pt-4">
