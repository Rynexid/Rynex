...
import { ArrowRight, Cloud, Code2, Palette, Plug, Smartphone } from "lucide-react";
          ...
          {services.map((service, i) => {
            ...
            return (
              <motion.div
                ...
                className={`group border-border bg-card hover:bg-card/80 relative overflow-hidden rounded-2xl border transition-[background-color,border-color] duration-500 hover:border-[#4FA3D1]/20 ${sizeClasses[service.size]}`}
              >
                <div
                  className={`relative z-10 flex h-full flex-col ${sizePadding[service.size]}`}
                >
                  <div
                    className={`mb-4 flex items-center justify-center rounded-xl bg-[#4FA3D1]/5 transition-colors group-hover:bg-[#4FA3D1]/10 ${
                      isLarge ? "h-14 w-14" : "h-11 w-11"
                    }`}
                  >
                    <service.icon
                      className={`text-[#4FA3D1] ${isLarge ? "h-7 w-7" : "h-5 w-5"}`}
                    />
                  </div>

                  <h3
                    className={`mb-2 font-bold tracking-tight transition-colors group-hover:text-[#4FA3D1] ${
                  ...
                  <Link href={service.href}>
                    <span className="inline-flex items-center gap-2 font-mono text-xs tracking-wider text-[#4FA3D1]/50 uppercase transition-colors group-hover:text-[#4FA3D1]">
                      Selengkapnya
